...
import { ArrowLeft, ArrowRight, Sun, Droplets, Layers, Check, Save } from "lucide-react";
...
export default function NewOrderPage() {
  ...
  // Step forms
  const step1 = useForm({ resolver: zodResolver(step1Schema), defaultValues: { customer_id: preselectedCustomer } });
  const solarForm = useForm({ resolver: zodResolver(solarSchema) });
  const tankForm = useForm({ resolver: zodResolver(tankSchema) });
  const pricingForm = useForm({ resolver: zodResolver(pricingSchema), defaultValues: { discount: 0, tax_rate: 0, subtotal: 0 } });
  ...
  const handleStep2 = async () => {
    ...
    if (valid) {
      ...
      setFormData((d) => ({ ...d, solar_detail: solarValues, tank_detail: tankValues }));
      setStep(2);
    }
  };
  ...
  const selectedCustomer = customers?.items.find((c) => c.id === formData.customer_id);
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Create a service order</p>
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 0 && (
          ...
          >
            <Card className="p-6 rounded-2xl space-y-5">
              <h2 className="text-base font-bold font-display">Select Customer & Service</h2>
              <Separator />
              <form onSubmit={handleStep1} className="space-y-5">
                <div className="space-y-1.5">
                  ...
                  {step1.formState.errors.customer_id && (
                    <p className="text-xs text-destructive">{step1.formState.errors.customer_id.message}</p>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { value: "SOLAR", label: "Solar", icon: Sun, color: "amber" },
                      { value: "TANK", label: "Tank", icon: Droplets, color: "blue" },
                      { value: "COMBINED", label: "Combined", icon: Layers, color: "green" },
                      ...
                      return (
                        <button
                          key={value}
                          type="button"
                          onClick={() => step1.setValue("service_type", value as any)}
                          className={cn(
                        ...
                        >
                          <Icon className={cn(
                            "w-6 h-6",
                            color === "amber" ? "text-amber-500" : color === "blue" ? "text-blue-500" : "text-green-500"
                          )} />
                  ...
                  {step1.formState.errors.service_type && (
                    <p className="text-xs text-destructive">Select a service type</p>
                  )}
                </div>
        ...
        {step === 1 && (
          ...
          >
            <Card className="p-6 rounded-2xl space-y-5">
              <h2 className="text-base font-bold font-display">Service Details</h2>
              <Separator />
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Input type="number" min={1} className="rounded-xl" {...solarForm.register("panel_count")} />
                    </div>
                    <div className="space-y-1.5">
                      <Label>Capacity (kW) *</Label>
                      <Input type="number" min={0.1} step={0.1} className="rounded-xl" {...solarForm.register("capacity_kw")} />
                    </div>
                    <div className="space-y-1.5">
                      <Label>Roof Type *</Label>
                      <Select onValueChange={(v) => solarForm.setValue("roof_type", v as any)}>
                        <SelectTrigger className="rounded-xl"><SelectValue placeholder="Select" /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="FLAT">Flat</SelectItem>
                          <SelectItem value="SLOPED">Sloped</SelectItem>
                          <SelectItem value="GROUND_MOUNTED">Ground Mounted</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <div className="space-y-1.5">
                      <Label>Panel Type *</Label>
                      <Select onValueChange={(v) => solarForm.setValue("panel_type", v as any)}>
                        <SelectTrigger className="rounded-xl"><SelectValue placeholder="Select" /></SelectTrigger>
                        <SelectContent>
                  ...
                  <div className="space-y-1.5">
                    <Label>Remarks</Label>
                    <Textarea className="rounded-xl resize-none" rows={2} {...solarForm.register("remarks")} />
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Select onValueChange={(v) => tankForm.setValue("tank_type", v as any)}>
                        <SelectTrigger className="rounded-xl"><SelectValue placeholder="Select" /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input type="number" min={1} className="rounded-xl" {...tankForm.register("capacity_liters")} />
                    </div>
                    <div className="space-y-1.5">
                      <Label>Number of Tanks *</Label>
                      <Input type="number" min={1} defaultValue={1} className="rounded-xl" {...tankForm.register("number_of_tanks")} />
                    </div>
                    <div className="space-y-1.5">
                      <Label>Chemical Used</Label>
                      <Input placeholder="e.g. Chlorine" className="rounded-xl" {...tankForm.register("chemical_used")} />
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <Label>Remarks</Label>
                    <Textarea className="rounded-xl resize-none" rows={2} {...tankForm.register("remarks")} />
                  </div>
                </div>
              )}
              ...
              <div className="flex justify-between">
                ...
                <Button onClick={handleStep2} className="rounded-xl shadow-premium">
                  Next <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </div>
            </Card>
        ...
        {step === 2 && (
          ...
          >
            <Card className="p-6 rounded-2xl space-y-5">
              <h2 className="text-base font-bold font-display">Scheduling & Pricing</h2>
              <Separator />
              <form onSubmit={handleStep3} className="space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label>Scheduled Date</Label>
                    <Input type="date" className="rounded-xl" {...pricingForm.register("scheduled_date")} />
                  </div>
                  <div className="space-y-1.5">
                    <Label>Scheduled Time</Label>
                    <Input type="time" className="rounded-xl" {...pricingForm.register("scheduled_time")} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <Input type="number" min={0} step={0.01} className="rounded-xl" {...pricingForm.register("subtotal")} />
                  </div>
                  <div className="space-y-1.5">
                    <Label>Discount (₹)</Label>
                    <Input type="number" min={0} step={0.01} defaultValue={0} className="rounded-xl" {...pricingForm.register("discount")} />
                  </div>
                  <div className="space-y-1.5">
                    <Label>Tax Rate (%)</Label>
                    <Input type="number" min={0} max={100} step={0.01} defaultValue={0} className="rounded-xl" {...pricingForm.register("tax_rate")} />
                  </div>
                </div>
                ...
                <div className="bg-muted/50 rounded-xl p-4 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between text-muted-foreground">
                    <span>Discount</span>
                    <span className="tabular-nums text-green-600">−{formatCurrency(discount)}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                ...
                <div className="space-y-1.5">
                  <Label>Notes</Label>
                  <Textarea className="rounded-xl resize-none" rows={2} placeholder="Any special instructions…" {...pricingForm.register("notes")} />
                </div>
        ...
        {step === 3 && (
          ...
          >
            <Card className="p-6 rounded-2xl space-y-5">
              <h2 className="text-base font-bold font-display">Review & Confirm</h2>
              <Separator />

              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Customer</span>
                  <span className="font-medium">{selectedCustomer?.name || formData.customer_id as string}</span>
                </div>
                <div className="flex justify-between">
                ...
                {(formData.scheduled_date as string | undefined) && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      ...
                      {(formData.scheduled_time as string | undefined) ? ` at ${formData.scheduled_time as string}` : ""}
                    </span>
                  </div>
                )}
                <Separator />
