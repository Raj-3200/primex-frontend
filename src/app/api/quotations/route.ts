...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ quotations, stats: { total: Number(statsRows[0].total), pending: Number(statsRows[0].pending), scheduled: Number(statsRows[0].scheduled), total_value: Number(statsRows[0].total_value) } });
  } catch (err) {
