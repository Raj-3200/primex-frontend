...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [orders, statsRows] = await Promise.all([
      ...
      sql`SELECT COUNT(*) AS total, COUNT(*) FILTER (WHERE status = 'COMPLETED') AS completed, COUNT(*) FILTER (WHERE status IN ('PENDING','SCHEDULED','IN_PROGRESS')) AS pending, COALESCE(SUM(total_amount) FILTER (WHERE status = 'COMPLETED'), 0) AS revenue FROM orders WHERE service_type = 'COMBINED' AND is_deleted = false`,
    ]);
    return NextResponse.json({ orders, stats: { total: Number(statsRows[0].total), completed: Number(statsRows[0].completed), pending: Number(statsRows[0].pending), revenue: Number(statsRows[0].revenue), active_contracts: Number(statsRows[0].pending), expiring_soon: 0 } });
  } catch (err) {
